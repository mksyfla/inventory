...
import { itemSchema, ItemFormValues } from "../../types/item";
...
export const ItemFormPage: React.FC = () => {
    ...
    // Real categories from the API (replaces the old hardcoded MOCK_CATEGORIES,
    // whose IDs never matched master.categories in the DB).
    ...
    // Auto-select the first category in create mode once categories load
    const categoryId = watch("categoryId");
    useEffect(() => {
        if (!isEditMode && !categoryId && categories.length > 0) {
            setValue("categoryId", categories[0].id);
        }
    }, [isEditMode, categoryId, categories, setValue]);
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <Space>
                                        ...
                                        <span>
                                            {field.value
                                                ? "Wajib Input Serial Number"
                                                : "Tidak Memakai Serial"}
                                        </span>
                                    </Space>
                                )}
                            />
                        ...
                        <Space align="center">
                            ...
                            <div>
                                <Title level={3} style={{ margin: 0 }}>
                                    {isEditMode
                                        ? `Edit Data Barang: ${existingItem?.sku}`
                                        : "Tambah Barang (SKU) Baru"}
                                </Title>
                                <Paragraph type="secondary" style={{ margin: 0 }}>
                                    Isi informasi master barang, spesifikasi UoM, batas stok, dan
                                    atribut pelacakan.
                                </Paragraph>
                            </div>
                        </Space>
